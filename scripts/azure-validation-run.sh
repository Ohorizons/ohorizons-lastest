...
Usage:
  ...
  scripts/azure-validation-run.sh --phase plan --run-id <run-id> --validation-scope infra --domain-name <temporary-domain>
  scripts/azure-validation-run.sh --phase apply --run-id <run-id> --confirm-apply
  scripts/azure-validation-run.sh --phase validate-all --run-id <run-id>
  scripts/azure-validation-run.sh --phase destroy --run-id <run-id> --confirm-destroy --destroy-confirm-text <client-name>-prod
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --validation-scope) VALIDATION_SCOPE="$2"; shift 2 ;;
...
on_unexpected_error() {
  local exit_code=$?
  if [[ "${TRAP_ACTIVE:-false}" == true ]]; then
    local line_no="${BASH_LINENO[0]:-unknown}"
    write_error "${PHASE:-unknown}" "unexpected_error" "deploy" "Unexpected script failure at line ${line_no} (exit ${exit_code}). Review phase artifacts and rerun after remediation." ""
    write_status "${PHASE:-unknown}" "failed" "unexpected_error" "deploy" true
  fi
  exit "$exit_code"
}
...
phase_plan() {
  ...
  local plan_exit=0
  if terraform plan "$(terraform_var_files | sed -n '1p')" "$(terraform_var_files | sed -n '2p')" -out="$PLAN_FILE" -detailed-exitcode -no-color > "$phase_dir/terraform-plan.log" 2>&1; then
    plan_exit=0
  else
    plan_exit=$?
  fi
  if grep -q '^Error:' "$phase_dir/terraform-plan.log"; then
  ...
  if command -v conftest >/dev/null 2>&1; then
    ...
    if conftest test "$PLAN_JSON" --policy "$PROJECT_DIR/policies/terraform" --all-namespaces > "$phase_dir/conftest.log" 2>&1; then
      conftest_exit=0
    else
      conftest_exit=$?
    fi
