...
`${{ values.name }}` was generated from the Open Horizons `mcp-spotify-backstage` Golden
...
MCP server exposing upstream Spotify Backstage documentation to agents.
...
- The path instructions under `.github/instructions/` are authoritative for the
  files they match: `devops-core-principles`, `markdown`, `security-and-owasp`, `self-explanatory-code-commenting`, `typescript-mcp-server`.
