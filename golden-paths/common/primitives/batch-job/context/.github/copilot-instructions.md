...
`${{ values.name }}` was generated from the Open Horizons `batch-job` Golden
...
Scheduled batch or cron workload with retry and observability.

**Primary stack:** Kubernetes CronJob
**Owner:** `${{ values.owner }}`

## How to work here

- Prefer the installed skills over improvising: `azure-kubectl-cli`, `azure-kubernetes`, `conventional-branch`, `documentation-writer`, `pipeline-diagnostics`, `review-and-refactor`, `security-review`, `story-planning`, `test-coverage`.
