...
`${{ values.name }}` was generated from the Open Horizons `basic-cicd` Golden
...
CI/CD pipeline added to an existing repository, with build, test, lint, security scanning, and deployment stages.
...
- Prefer the installed skills over improvising: `conventional-branch`, `copilot-pr-autopilot`, `documentation-writer`, `git-commit`, `pipeline-diagnostics`, `review-and-refactor`, `security-review`, `story-planning`, `test-coverage`.
- The path instructions under `.github/instructions/` are authoritative for the
  files they match: `devops-core-principles`, `github-actions-ci-cd-best-practices`, `security-and-owasp`, `self-explanatory-code-commenting`.
