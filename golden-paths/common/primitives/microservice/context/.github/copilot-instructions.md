...
`${{ values.name }}` was generated from the Open Horizons `microservice` Golden
...
Complete containerised microservice with API, observability, and GitOps deployment.
...
- Prefer the installed skills over improvising: `api-testing`, `azure-kubernetes`, `conventional-branch`, `documentation-writer`, `multi-stage-dockerfile`, `pipeline-diagnostics`, `review-and-refactor`, `security-review`, `story-planning`, `test-coverage`.
