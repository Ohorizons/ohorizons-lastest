...
`${{ values.name }}` was generated from the Open Horizons `api-gateway` Golden
...
API gateway configuration with routing, authentication, rate limiting, and policy.

**Primary stack:** Azure API Management, Kong
**Owner:** `${{ values.owner }}`

## How to work here

- Prefer the installed skills over improvising: `api-testing`, `apim-ai-gateway`, `azure-api-center`, `contract-testing`, `conventional-branch`, `documentation-writer`, `pipeline-diagnostics`, `review-and-refactor`, `security-review`, `story-planning`, `test-coverage`.
- The path instructions under `.github/instructions/` are authoritative for the
  files they match: `azure-apim-ai-gateway`, `devops-core-principles`, `security-and-owasp`, `self-explanatory-code-commenting`.
