...
`${{ values.name }}` was generated from the Open Horizons `mcp-agent-framework` Golden
...
MCP server exposing Microsoft Agent Framework knowledge to agents.
...
- Prefer the installed skills over improvising: `conventional-branch`, `documentation-writer`, `mcp-builder`, `mcp-cli`, `mcp-implementation-security-review`, `microsoft-agent-framework`, `pipeline-diagnostics`, `review-and-refactor`, `security-review`, `story-planning`, `test-coverage`.
