...
`${{ values.name }}` was generated from the Open Horizons `mcp-gh-aw` Golden
...
MCP server exposing GitHub Agentic Workflows to agents.
...
- The path instructions under `.github/instructions/` are authoritative for the
  files they match: `devops-core-principles`, `github-actions-ci-cd-best-practices`, `security-and-owasp`, `self-explanatory-code-commenting`, `typescript-mcp-server`.
