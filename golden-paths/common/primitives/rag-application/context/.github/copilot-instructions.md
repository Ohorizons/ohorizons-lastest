...
`${{ values.name }}` was generated from the Open Horizons `rag-application` Golden
...
Retrieval-augmented generation application with vector search, ingestion, and grounding checks.
...
- Prefer the installed skills over improvising: `agentic-eval`, `azure-agentic-architecture-patterns`, `azure-ai`, `conventional-branch`, `documentation-writer`, `pinecone-rag`, `pipeline-diagnostics`, `review-and-refactor`, `security-review`, `story-planning`, `test-coverage`.
- The path instructions under `.github/instructions/` are authoritative for the
  files they match: `ai-prompt-engineering-safety-best-practices`, `devops-core-principles`, `langchain-python`, `microsoft-foundry`, `security-and-owasp`, `self-explanatory-code-commenting`.
