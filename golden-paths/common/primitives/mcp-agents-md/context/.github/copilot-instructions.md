...
`${{ values.name }}` was generated from the Open Horizons `mcp-agents-md` Golden
...
MCP server exposing the AGENTS.md format and conventions to agents.
...
- Prefer the installed skills over improvising: `conventional-branch`, `create-agentsmd`, `documentation-writer`, `mcp-builder`, `mcp-cli`, `mcp-implementation-security-review`, `pipeline-diagnostics`, `review-and-refactor`, `security-review`, `story-planning`, `test-coverage`.
- The path instructions under `.github/instructions/` are authoritative for the
  files they match: `agents`, `devops-core-principles`, `security-and-owasp`, `self-explanatory-code-commenting`, `typescript-mcp-server`.
