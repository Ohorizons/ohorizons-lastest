...
`${{ values.name }}` was generated from the Open Horizons `mcp-backstage-ui` Golden
...
MCP server exposing the Backstage design system to agents.
...
- The path instructions under `.github/instructions/` are authoritative for the
  files they match: `a11y`, `devops-core-principles`, `security-and-owasp`, `self-explanatory-code-commenting`, `typescript-mcp-server`.
