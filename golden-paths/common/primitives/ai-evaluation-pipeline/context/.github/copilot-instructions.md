...
`${{ values.name }}` was generated from the Open Horizons `ai-evaluation-pipeline` Golden
...
Model and agent evaluation pipeline with datasets, metrics, and regression gates.
...
- Prefer the installed skills over improvising: `agentic-eval`, `azure-ai`, `conventional-branch`, `documentation-writer`, `eval-driven-dev`, `phoenix-evals`, `pipeline-diagnostics`, `review-and-refactor`, `security-review`, `story-planning`, `test-coverage`.
- The path instructions under `.github/instructions/` are authoritative for the
  files they match: `ai-prompt-engineering-safety-best-practices`, `devops-core-principles`, `microsoft-foundry`, `security-and-owasp`, `self-explanatory-code-commenting`.
