...
`${{ values.name }}` was generated from the Open Horizons `mcp-spec-kit` Golden
...
MCP server exposing specification-driven development methodology to agents.
...
- Prefer the installed skills over improvising: `conventional-branch`, `documentation-writer`, `mcp-builder`, `mcp-cli`, `mcp-implementation-security-review`, `pipeline-diagnostics`, `review-and-refactor`, `security-review`, `story-planning`, `test-coverage`.
- The path instructions under `.github/instructions/` are authoritative for the
  files they match: `devops-core-principles`, `security-and-owasp`, `self-explanatory-code-commenting`, `spec-driven-workflow-v1`, `typescript-mcp-server`.
