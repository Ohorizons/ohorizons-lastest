...
`${{ values.name }}` was generated from the Open Horizons `mcp-backstage-plugins` Golden
...
MCP server exposing the Backstage plugin ecosystem to agents.
...
- The path instructions under `.github/instructions/` are authoritative for the
  files they match: `devops-core-principles`, `security-and-owasp`, `self-explanatory-code-commenting`, `typescript-mcp-server`.
