...
`${{ values.name }}` was generated from the Open Horizons `infrastructure-provisioning` Golden
...
Azure infrastructure-as-code project with policy compliance, cost controls, and drift detection.
...
- Prefer the installed skills over improvising: `azure-cli`, `azure-infrastructure`, `azure-terraform-cli`, `azure-validate`, `conventional-branch`, `documentation-writer`, `pipeline-diagnostics`, `review-and-refactor`, `security-review`, `story-planning`, `test-coverage`.
- The path instructions under `.github/instructions/` are authoritative for the
  files they match: `azure-naming`, `azure-verified-modules-terraform`, `bicep-code-best-practices`, `devops-core-principles`, `security-and-owasp`, `self-explanatory-code-commenting`, `terraform-azure`.
