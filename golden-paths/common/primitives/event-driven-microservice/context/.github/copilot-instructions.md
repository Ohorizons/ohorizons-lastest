...
`${{ values.name }}` was generated from the Open Horizons `event-driven-microservice` Golden
...
Event-driven service consuming and publishing through a message broker.
...
- Prefer the installed skills over improvising: `api-testing`, `azure-messaging`, `contract-testing`, `conventional-branch`, `documentation-writer`, `pipeline-diagnostics`, `review-and-refactor`, `security-review`, `story-planning`, `test-coverage`.
