...
`${{ values.name }}` was generated from the Open Horizons `gitops-deployment` Golden
...
GitOps delivery configuration with progressive rollout and drift reconciliation.
...
- Prefer the installed skills over improvising: `azure-argocd-cli`, `azure-helm-cli`, `azure-kubectl-cli`, `conventional-branch`, `devops-rollout-plan`, `documentation-writer`, `pipeline-diagnostics`, `review-and-refactor`, `security-review`, `story-planning`, `test-coverage`.
- The path instructions under `.github/instructions/` are authoritative for the
  files they match: `devops-core-principles`, `kubernetes-deployment-best-practices`, `kubernetes-manifests`, `security-and-owasp`, `self-explanatory-code-commenting`.
