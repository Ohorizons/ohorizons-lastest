...
if [[ $THREAT_COUNT -gt 0 ]]; then
  ...
  if [[ "$MODE" == "block" ]]; then
    echo "[BLOCKED] Operation blocked: resolve the threats above or adjust TOOL_GUARD_ALLOWLIST." >&2
    echo "   Set GUARD_MODE=warn to log without blocking." >&2
    exit 2
  else
    echo "[WARN] Threats logged in warn mode. Set GUARD_MODE=block to prevent dangerous operations." >&2
  fi
else
  # Log clean result
