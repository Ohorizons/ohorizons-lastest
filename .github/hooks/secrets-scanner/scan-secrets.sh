...
#   SECRETS_LOG_DIR    - Directory for scan logs (default: Copilot/XDG user state)
...
if [[ "${SKIP_SECRETS_SCAN:-}" == "true" ]]; then
  echo "[SKIP] Secrets scan skipped (SKIP_SECRETS_SCAN=true)"
  exit 0
fi

# Ensure we are in a git repository
if ! git rev-parse --is-inside-work-tree &>/dev/null; then
  echo "[WARN] Not in a git repository, skipping secrets scan"
  exit 0
fi
...
default_log_dir() {
  if [[ -n "${COPILOT_HOME:-}" ]]; then
    printf '%s/hook-logs/secrets-scanner' "$COPILOT_HOME"
  elif [[ -n "${XDG_STATE_HOME:-}" ]]; then
    printf '%s/github-copilot/hook-logs/secrets-scanner' "$XDG_STATE_HOME"
  elif [[ -n "${HOME:-}" ]]; then
    printf '%s/.local/state/github-copilot/hook-logs/secrets-scanner' "$HOME"
  else
...
LOG_DIR="${SECRETS_LOG_DIR:-$(default_log_dir)}"
...
if [[ ${#FILES[@]} -eq 0 ]]; then
  echo "[OK] No modified files to scan"
...
echo "[SCAN] Scanning ${#FILES[@]} modified file(s) for secrets..."
...
if [[ $FINDING_COUNT -gt 0 ]]; then
  echo ""
  echo "[WARN] Found $FINDING_COUNT potential secret(s) in modified files:"
  echo ""
  ...
  if [[ "$MODE" == "block" ]]; then
    echo "[BLOCKED] Session blocked: resolve the findings above before committing."
    echo "   Set SCAN_MODE=warn to log without blocking, or add patterns to SECRETS_ALLOWLIST."
    exit 2
  else
    echo "[HINT] Review the findings above. Set SCAN_MODE=block to prevent commits with secrets."
  fi
else
  echo "[OK] No secrets detected in ${#FILES[@]} scanned file(s)"
