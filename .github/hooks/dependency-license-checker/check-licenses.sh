...
if [[ "${SKIP_LICENSE_CHECK:-}" == "true" ]]; then
  echo "[SKIP] License check skipped (SKIP_LICENSE_CHECK=true)"
  exit 0
fi

# Ensure we are in a git repository
if ! git rev-parse --is-inside-work-tree &>/dev/null; then
  echo "[WARN] Not in a git repository, skipping license check"
  exit 0
fi
...
if [[ $FINDING_COUNT -gt 0 ]]; then
  echo "[WARN] Found $FINDING_COUNT license violation(s):"
  echo ""
  for violation in "${VIOLATIONS[@]}"; do
    IFS=$'\t' read -r pkg ecosystem license status <<< "$violation"
  ...
  if [[ "$MODE" == "block" ]]; then
    echo "[BLOCKED] Session blocked: resolve license violations above before committing."
    echo "   Set LICENSE_MODE=warn to log without blocking, or add packages to LICENSE_ALLOWLIST."
    exit 2
  else
    echo "[HINT] Review the violations above. Set LICENSE_MODE=block to prevent commits with license issues."
  fi
else
  echo "[OK] All ${#RESULTS[@]} dependencies have compliant licenses"
fi
